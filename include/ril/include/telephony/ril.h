 ...
 *                    New commands added: RIL_REQUEST_SET_ALLOWED_CARRIERS,
 *                    RIL_REQUEST_GET_ALLOWED_CARRIERS and RIL_UNSOL_PCO_DATA.
 *
 ...
 *                    New data structure RIL_CarrierRestrictionsWithPriority
 */
...
typedef enum {
    ...
    // MTK-START: SUBSIDYLOCK
    RIL_E_REBOOT_REQUIRED = 8000,
    RIL_E_VALIDATION_FAILED = 8001,
    RIL_E_CONFIG_DATA_INVALID = 8002,
    RIL_E_IMEI_MISMATCH = 8003,
    RIL_E_LENGTH_MISMATCH = 8004,
    RIL_E_UNKNOWN_ERROR = 8005,
    // MTK-END
...
typedef struct {
    ...
    int             speechCodec;/* Speech codec information */
} RIL_Call;
...
typedef enum {
    ...
    RIL_MATCH_MAX               /* It should be at the last line for max type value */
...
typedef struct {
    ...
    RIL_CarrierMatchType match_type;   /* Specify match type for the carrier.
                                        * If it is RIL_MATCH_ALL, match_data is null;
...
typedef struct {
    ...
    RIL_Carrier * allowed_carriers;     /**
                                         ...
                                         * The character '?' is used as wildcard character to
                                         * match any value.
                                         */
    ...
    bool allowedCarriersPrioritized;    /**
                                         * True means that only carriers included in the allowed
                                         * list and not in the excluded list are permitted. Eg.
                                         * allowedCarriers match mcc/mnc, excludedCarriers has
                                         * same mcc/mnc and gid1 is ABCD. It means except the
                                         * carrier whose gid1 is ABCD, all carriers with the
                                         * same mcc/mnc are allowed.False means that all carriers
                                         * are allowed except those included in the excluded list
                                         * and not in the allowed list.
                                         */
    int simLockMultiSimPolicy;
...
/* See RIL_REQUEST_SET_CALL_INDICATION */
typedef enum {
    CALL_INDICATION_CAUSE_INCOMING_REJECT = 16, /* Call indication with cause "reject" */

    CALL_INDICATION_CAUSE_UNSPECIFIED = 0xffff /* Call indication cause unspecified. */
} RIL_CallIndicationCause;

/* See RIL_REQUEST_HANGUP */
// these IDs need to match MtkDisconnectCause.java
typedef enum {
    INCOMING_REJECTED_NO_FORWARD = 1007, /* Disconnect cause for reject call. */

    INCOMING_REJECTED_UNSPECIFIED = 0xffff /* Disconnect cause unspecified. */
} RIL_DisconnectCause;
...
typedef enum {
    ...
    RIL_DATA_PROFILE_VENDOR_WAP        = RIL_DATA_PROFILE_OEM_BASE + 4,
    RIL_DATA_PROFILE_VENDOR_EMERGENCY  = RIL_DATA_PROFILE_OEM_BASE + 5,
    RIL_DATA_PROFILE_VENDOR_XCAP       = RIL_DATA_PROFILE_OEM_BASE + 6,
    RIL_DATA_PROFILE_VENDOR_RCS        = RIL_DATA_PROFILE_OEM_BASE + 7,
    RIL_DATA_PROFILE_VENDOR_BIP        = RIL_DATA_PROFILE_OEM_BASE + 8,
    RIL_DATA_PROFILE_VENDOR_VSIM       = RIL_DATA_PROFILE_OEM_BASE + 9,
    RIL_DATA_PROFILE_VENDOR_MCX        = RIL_DATA_PROFILE_OEM_BASE + 10,
    RIL_DATA_PROFILE_VENDOR_ALL        = RIL_DATA_PROFILE_OEM_BASE + 11,
    // MTK end
...
typedef enum {
    ...
    // Define base persosubstate.
    RIL_PERSOSUBSTATE_SIM_NS_SP                 = 25,
    RIL_PERSOSUBSTATE_SIM_NS_SP_PUK             = 26,
    RIL_PERSOSUBSTATE_SIM_SIM_C                 = 27,
    RIL_PERSOSUBSTATE_SIM_SIM_C_PUK             = 28,
} RIL_PersoSubstate;
...
typedef enum {
    NO_MULTISIM_POLICY = 0,             /**
                                         * Indicates that configuration applies to each slot
                                         * independently.
                                         */

    ONE_VALID_SIM_MUST_BE_PRESENT = 1,  /**
                                         * Indicates that any SIM card can be used as far as one
                                         * valid card is present in the device. For the modem, a
                                         * SIM card is valid when its content(i.e. MCC, MNC, GID,
                                         * SPN) matches the carrier restriction configuration.
                                         */
...
/* Deprecated, use RIL_CardStatus_v7 */
typedef struct
{
  RIL_CardState card_state;
  RIL_PinState  universal_pin_state;             /* applicable to USIM and CSIM: RIL_PINSTATE_xxx */
...
/* Deprecated, use RIL_CardStatus_v8 */
typedef struct
{
  RIL_CardState card_state;
  RIL_PinState  universal_pin_state;             /* applicable to USIM and CSIM: RIL_PINSTATE_xxx */
...
typedef struct {
    RIL_NrSignalStrength signalStrength;
...
typedef struct {
    ...
    // [Network][C2K] MTK_TC1_FEATURE
    int32_t dataRoamingIndicator;         // Data Roaming indicator
                                          // Valid values are 0-255.
    int32_t dataSystemIsInPrl;            // The information if the current system is in the PRL for Data
                                          // 0=not in the PRL, 1=in the PRL
    int32_t dataDefaultRoamingIndicator;  // The default Roaming Indication from the PRL for Data
                                          // Valid values are 0-255.
...
typedef enum {
    ...
    RIL_APN_TYPE_MCX          = 0x400,        // APN type for MCX

    RIL_APN_TYPE_WAP          = 0x800,        // APN type for WAP
    RIL_APN_TYPE_XCAP         = 0x1000,       // APN type for XCAP
    RIL_APN_TYPE_RCS          = 0x2000,       // APN type for RCS
    RIL_APN_TYPE_BIP          = 0x4000,       // APN type for BIP
    RIL_APN_TYPE_VSIM         = 0x8000,       // APN type for VSIM

    RIL_APN_TYPE_COUNT        = 16,
    ...
    RIL_APN_TYPE_ALL          = RIL_APN_TYPE_DEFAULT | RIL_APN_TYPE_MMS | RIL_APN_TYPE_SUPL |
                                ...
                                RIL_APN_TYPE_EMERGENCY | RIL_APN_TYPE_MCX,

    RIL_APN_TYPE_MTKALL       = RIL_APN_TYPE_ALL | RIL_APN_TYPE_WAP | RIL_APN_TYPE_XCAP |
                                RIL_APN_TYPE_RCS | RIL_APN_TYPE_BIP | RIL_APN_TYPE_VSIM
} RIL_ApnTypes;
...
typedef struct {
    int specifyChannels;
    uint32_t specifiers_length;
    RIL_RadioAccessSpecifier specifiers[MAX_RADIO_ACCESS_NETWORKS];
} RIL_SystemSelectionChannels;
...
typedef struct {
    ...
    int num_cids;
    int32_t contextIds[16];
    uint32_t physicalCellId;
} RIL_PhysicalChannelConfig;

typedef enum {
    NETWORK_SIGNALING = 0x01, // Indicates the number is from the network signal
    SIM               = 0x02, // Indicates the number is from the sim card.
    MODEM_CONFIG      = 0x04, // Indicates the number is from the modem config
    DEFAULT           = 0x08, // Indicates the number is available as default
} RIL_EmergencyNumberSource;

typedef struct {
    char* number;       // The emergency number
    char* mcc;          // 3-digit Mobile Country Code, 0..999. Empty string if not applicable
    char* mnc;          // 2 or 3-digit Mobile Network Code, 0..999. Empty string if not applicable
    uint32_t categories;// The bitfield of @1.4::EmergencyServiceCategory(s)
    char** urns;        // The list of emergency Uniform Resource Names (URN)
    uint32_t sources;   // The bitfield of @1.4::EmergencyNumberSource(s)
} RIL_EmergencyNumber;
 ...
 *  CANCELLED
 */
#define RIL_REQUEST_SET_ALLOWED_CARRIERS 136
 ...
 *  CANCELLED
 */
#define RIL_REQUEST_GET_ALLOWED_CARRIERS 137
...
#define RIL_REQUEST_GET_SLOT_STATUS 144
#define RIL_REQUEST_SET_LOGICAL_TO_PHYSICAL_SLOT_MAPPING 145
 ...
 *
 */
#define RIL_REQUEST_START_KEEPALIVE 146
 ...
 *
 */
#define RIL_REQUEST_STOP_KEEPALIVE 147

// MTK-START add before AOSP defines one day.
#define RIL_REQUEST_SET_SIGNAL_STRENGTH_REPORTING_CRITERIA 148
#define RIL_REQUEST_SET_LINK_CAPACITY_REPORTING_CRITERIA 149
#define RIL_REQUEST_SET_PREFERRED_DATA_MODEM 150
// MTK-END

#define RIL_REQUEST_EMERGENCY_DIAL 151
#define RIL_REQUEST_ENABLE_MODEM 152
#define RIL_REQUEST_SET_SYSTEM_SELECTION_CHANNELS 153

#define RIL_REQUEST_GET_PHONE_CAPABILITY 154
...
#define RIL_UNSOL_ICC_SLOT_STATUS 1050
 ...
 */
#define RIL_UNSOL_KEEPALIVE_STATUS 1051

/**
 * RIL_UNSOL_EMERGENCY_NUMBER_LIST
 *
 * "data" is NULL
 * "response" is an array of RIL_EmergencyNumber
 */
...
typedef void (*RIL_UpdateConnectiontState) (RIL_SOCKET_ID socketId, int isConnected);
...
typedef struct {
    ...
    RIL_UpdateConnectiontState updateConnectionState;
} RIL_RadioFunctions;

typedef enum {
    CONNECTION_STATE_OFF = 0,
    CONNECTION_STATE_ON = 1,
} CONNECTION_STATE;
...
#define MAX_INADDR_LEN 65 // Should be same as MAX_IPV6_ADDRESS_LENGTH
typedef struct {
...
// maps to radio/config/1.1/types.hal
typedef struct {
    /**
     * Logical modem ID.
     */
...
typedef struct {
     ...
     * other one.
     */
    uint8_t isInternetLingeringSupported;
    /**
     * List of logical modem information.
     */
    RIL_ModemInfo logicalModemList[SIM_COUNT];
} RIL_PhoneCapability;
...
struct RIL_Env {
    ...
    */
    ...
    /**
     * Query Executing Context from RIL_Token
     */
    RILChannelId (*QueryMyChannelId) (RIL_Token t);
#ifndef C2K_RIL
    /**
     * Query Request Context from RIL_Token
     */
    RILChannelId (*QueryCommandChannelId) (RIL_Token t);
#endif
    /**
